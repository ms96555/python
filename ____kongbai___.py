...
if __name__ == '__main__':
    ...
    code = '11111132232323'
    key = b'85080891afadfad'
    gw = blowfish()
    print
    "明文密码：%s，经过key：%s加密之后的加密密码是：%s" % (code, key, gw.Encrypt(code, key))
    encode = gw.Encrypt(code, key)
    print
    "加密密码：%s，经过key：%s解密之后的明文密码是：%s" % (encode, key, gw.Decrypt(encode, key))
